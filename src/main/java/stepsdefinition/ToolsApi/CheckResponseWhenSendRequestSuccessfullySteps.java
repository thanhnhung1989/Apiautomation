...
//	@When("I send tool request with category {string} and results {int}")
//	public void i_send_tool_request_with_category(String category, int results) {
//	    try {
//	        // Thêm tham số query vào URL
//	        String query = String.format("?category=%s&results%d", category, results);
...
//	    }
//	}
	@When("I send tool request with param {string}")
	public void i_send_tool_request_with_category_and_results(String category) {
	    try {
	        // Xây dựng URL với tham số category và results
	        String query = String.format("?category=%s", category);
	        givenurl = "https://simple-tool-rental-api.glitch.me/tools" + query;
	...
	@Then("Response body matches category {string}")
	public void response_body_matches_category(String category) {
